...
CHECKIN_URL = "https://ikuuu.org/user/checkin"
# =============================================
...
def checkin(cookie_str: str) -> dict:
    """
    执行签到
    返回 {"success": True, "msg": "...", "data": {...}} 或 {"success": False, "error": "..."}
    """
    headers = {
    ...
    try:
        response = requests.post(CHECKIN_URL, headers=headers, impersonate="chrome124")
        ...
        if result.get("ret") == 1:
            ...
            return {"success": False, "error": result.get("msg", "签到失败")}
